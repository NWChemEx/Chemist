...
#include <chemist/basis_set2/shell/detail_/shell_pimpl.hpp>

using namespace chemist::basis_set;
using namespace chemist::basis_set::detail_;

TEMPLATE_TEST_CASE("ShellPIMPL", "", float, double) {
    ...
    using pimpl_type   = ShellPIMPL<cg_type>;
    ...
    SECTION("Ctors") {
        SECTION("Defaulted") {
            REQUIRE(defaulted.m_cg == cg0);
            REQUIRE(defaulted.m_l == l0);
            REQUIRE(defaulted.m_pure == pure);
        }
        SECTION("With Values") {
