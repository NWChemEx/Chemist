...
TEMPLATE_TEST_CASE("Charges", "", float, double) {
    ...
    for(std::size_t charge_i = 0; charge_i < 3; ++charge_i) {
        old_addresses.push_back(&charges[charge_i].charge());
        for(std::size_t coord_i = 0; coord_i < 3; ++coord_i) {
            old_addresses.push_back(&charges[charge_i].coord(coord_i));
        }
    }
    ...
    SECTION("Ctor") {
        ...
        SECTION("Move") {
            ...
            for(std::size_t charge_i = 0; charge_i < 3; ++charge_i) {
                ...
                REQUIRE(old_addresses[idx] == &move1[charge_i].charge());
                for(std::size_t coord_i = 0; coord_i < 3; ++coord_i) {
                    const auto p = &move1[charge_i].coord(coord_i);
                    REQUIRE(old_addresses[idx + 1 + coord_i] == p);
                }
            }
        }
    ...
    SECTION("at_()") {
        ...
        for(std::size_t charge_i = 0; charge_i < 3; ++charge_i) {
            ...
            REQUIRE(old_addresses[idx] == &qi.charge());

            for(std::size_t coord_i = 0; coord_i < 3; ++coord_i) {
                auto& coord = qi.coord(coord_i);

                REQUIRE(coord == corr.coord(coord_i));
                REQUIRE(old_addresses[idx + 1 + coord_i] == &coord);
            }
        }
    }
