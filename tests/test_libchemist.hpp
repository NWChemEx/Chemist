...
inline auto generate_tensor(unsigned int rank, double seed = 1.0) {
    ...
    using vector_il = TA::vector_il<double>;
    using matrix_il = TA::matrix_il<double>;
    using tensor_il = TA::tensor_il<double>;
    
    if(rank == 1u) {
        t = ta_tensor_type(world, vector_il{seed, seed + 1.0, seed + 2.0});
    } else if(rank == 2u) {
        t = ta_tensor_type(world, matrix_il{vector_il{seed + 0.0, seed + 1.0, seed + 2.0},
                                   vector_il{seed + 3.0, seed + 4.0, seed + 5.0},
                                   vector_il{seed + 6.0, seed + 7.0, seed + 8.0}});
    } else if(rank == 3ul) {
        t = ta_tensor_type(world, tensor_il{matrix_il{vector_il{seed + 00.0, seed + 01.0, seed + 02.0},
