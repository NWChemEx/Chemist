...
TEST_CASE("make_reduced_tile_") {
    ...
    SECTION("Vector-of-vectors to vector"){
        ...
        SECTION("Incompatible ranks") {
            ...
            SparseMap<ei, ei> bad_sm(
                    {{e0, {e01, e12}}, {e1, {e02, e12}}});
            tile_type tile(TA::Range{1});
            REQUIRE_THROWS_AS(make_reduced_tile_(tile, bad_sm, trange, t_of_t),std::runtime_error);
        }
    }
}
...
TEST_CASE("reduce_tot_sum") {
    ...
    SECTION("Vector-of-vectors to vector") {
        ...
        SECTION("Incompatible ranks") {
            ...
            REQUIRE_THROWS_AS(reduce_tot_sum(bad_ind_sm, t_of_t, trange), std::runtime_error);

            SparseMap<ei, ei> bad_dep_sm(
                    {{e0, {e01, e12}}, {e1, {e02, e12}}});
            REQUIRE_THROWS_AS(reduce_tot_sum(bad_dep_sm, t_of_t, trange), std::runtime_error);

            TA::TiledRange bad_trange{{0, 2, 4}, {0,2,4}};
            REQUIRE_THROWS_AS(reduce_tot_sum(sm, t_of_t, bad_trange), std::runtime_error);
        }
    }
}