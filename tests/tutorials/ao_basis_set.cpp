...
// tutorial to be self contained (and because someone, somewhere, must make the
// ``AOBasisSet`` instance you are using) we start by showing how to create an
// ``AOBasisSet`` instance.
//
...
// the STO-3G parameters for a water molecule. If you are not interested in how
// to make an ``AOBasisSet`` instance skip to the next code block.
...
TEST_CASE("AOBasisSet Tutorial") {
    ...
    libchemist::Center<double> H1(1.6380335020342418, 1.1365568803584036, 0.0);
    ...
    libchemist::Center<double> H2(-1.6380335020342418, 1.1365568803584036, 0.0);
    ...
    //    classes which would combine one, or more, ``ContractedGaussian`` with
    ...
    // it at a specific coarseness. For example we may want to loop over unique
    ...
    // TUTORIAL
    //
    // It is arguably easier to reason about a basis set if we think of it as an
    // "array of structures" instead of a "structure of arrays". The problem is
    // that the "array-of-structures" view is inconsistent with the API of most
    ...
    // (the views) we avoid the need to copy the data into temporary structures
    // simply to get the desired "array of structures" API. Furthermore, by
    // using a view/non-view pair the memory semantics are always clear thereby
    ...
    // implicitly converted to a read-only, non-view variant (which in turn can
    // be implicitly converted into a read-/write-able non-view variant by a
    // deep copy). This allows views to be used with functions that are defined
