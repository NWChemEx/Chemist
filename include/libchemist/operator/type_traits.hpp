...
/** Type trait to determine if a type is an density dependent operator type at compile-time
 *
 *  @tparam OpType The type to check
 *
 *  Creates a bool_constant which is `true` if `OpType` is derived from DensityDependentOperator,
 *  `false`, otherwise.
 */
template <typename OpType>
...
/** Type trait to determine if a type is an density independent operator type at compile-time
 *
 *  @tparam OpType The type to check
 *
 *  Creates a bool_constant which is `true` if `OpType` is derived from Operator but not 
...
/// Checks if all operators in parameter pack are density dependent operator types
template <typename... Ops>
inline constexpr bool all_are_density_dependent_operator_v = 
  std::conjunction_v< is_density_dependent_operator<Ops>... >;

/// Checks if all operators in parameter pack are density independent operator types
template <typename... Ops>
...
/** Type trait to check if a type is an operator type which takes exactly `N` electrons.
 *
 ...
 *  @tparam OpType Type to check
 *
 *  Creates a bool_constant which is `true` if the passed type is an Operator type
 *  and OpType::n_electrons == N. `false` otherwise.
 */
