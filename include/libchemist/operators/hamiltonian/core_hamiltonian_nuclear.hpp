 ...
 *
 *  The core (electronic) Hamiltonian derivative contains terms which are one-electron
 *  terms or sums of one-electron terms. In practice we rarely need the many-
 *  electron core Hamiltonian derivative so by convention this class is the one-electron
...
public:
    ...
    CoreHamiltonian_Nuclear& operator=(const CoreHamiltonian_Nuclear&) = default;
    CoreHamiltonian_Nuclear& operator=(CoreHamiltonian_Nuclear&&) = default;

    /** @brief Convenience ctor for creating a CoreHamiltonian derivative from the
     *         ElectronicHamiltonian derivative.
     *
     *  This function will initialize and add an ElectronKinetic derivative operator
     *  if an NElectronKinetic derivative operator is present in @p He, then loop over
     *  the NElectronNuclearAttraction derivative operators in @p He and initialize and
     *  add corresponding ElectronNuclearAttraction derivative operators for each.
     *
