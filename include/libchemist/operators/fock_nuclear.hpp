...
/** @brief A class to store and manipulate Fock Derivative operators.
 *
 ...
 *
 */
class Fock_Nuclear : public detail_::LinearCombination {
private:
    using base_type = detail_::LinearCombination;

    template<typename T>
    constexpr static bool is_me_v = std::is_same_v<Fock_Nuclear, T>;

    template<typename T>
    using disable_if_me_t = std::enable_if_t<!is_me_v<T>>;

public:
    /** @brief Creates a new Fock Derivative instance containing no operators.
     *
     ...
     */
    Fock_Nuclear(const Fock_Nuclear& other);
     ...
     *  Complexity: Constant.
     */
    Fock_Nuclear(Fock_Nuclear&& other) noexcept;

    /// Defaulted no-throw dtor
    virtual ~Fock_Nuclear() noexcept;
     ...
     *  Complexity: Linear in the number of operators which comprise @p other.
     */
    Fock_Nuclear& operator=(const Fock_Nuclear& other);
     ...
     *  Complexity: Constant.
     */
    Fock_Nuclear& operator=(Fock_Nuclear&& other) noexcept;
    ...
    explicit Fock_Nuclear(OpType&& op0, Args&&... args);
...
Fock_Nuclear::Fock_Nuclear(OpType&& op0, Args&&... args) : base_type() {
