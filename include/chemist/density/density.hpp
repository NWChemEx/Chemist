 ...
 *
 */
template<typename OrbitalSpace, typename... Particles>
class Density {
public:
    ...
    using base_space_type = OrbitalSpace;

    /// Default ctor
    explicit Density() = default;

    /// Ctor with non-default values
    Density(value_type rho, base_space_type space) :
      m_orbs_(std::move(space)), m_density_(std::move(rho)) {}
...
protected:
     ...
     *  @throw None No throw guarantee.
     */
    virtual const base_space_type& basis_set_() const { return m_orbs_; }
 ...
 *
 *  @tparam LHSTypes The particle types for @p lhs.
 *  @tparam RHSTypes The particle types for @p rhs.
 *
...
bool operator==(const Density<LHSTypes...>& lhs,
                const Density<RHSTypes...>& rhs) {
...
/// Type of the one-Electron density (1RDM)
template <typename OrbitalSpace = orbital_space::AOSpaceD>
using OneElectronDensity = Density<OrbitalSpace, Electron>;

// Type of the two-Electron density (2RDM)
template <typename OrbitalSpace = orbital_space::DerivedSpaceD>
using TwoElectronDensity = Density<OrbitalSpace, Electron, Electron>;
