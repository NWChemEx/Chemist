...
class ContractedGaussianPIMPL {
  ...
  ContractedGaussianPIMPL(coefficient_vector cs,
                          exponent_vector exps,
                          center_type r0):
    m_center_(std::move(r0)), m_coefs_(std::move(cs)), m_exp_(std::move(exp)) {}
...
CG::ContractedGaussian(std::vector<coefficient_type> coefs,
  ...
  ContractedGaussian(
    m_pimpl_(
      std::make_unique<pimpl_type>(std::move(coefs), 
                                   std::move(exps), 
                                   std::move(center)))) {}
...
CG::ContractedGaussian(pimpl_ptr my_pimpl) noexcept :
  utilities::IndexableContainerBase<CG>(),
...
void CG::assert_pimpl_() const {
  ...
  throw std::runtime_error("ContractedGaussian does not have a PIMPL. This is"
  " usually the case if it was default constructed or moved from.");
}

template<typename PrimitiveType>
typename CG::size_type CG::size_()
  const noexcept {
    if(is_null()) return 0;
...
template<typename PrimitiveType>
typename CG::reference CG::at_(size_type i)  noexcept { return (*m_pimpl_)[i]; }
