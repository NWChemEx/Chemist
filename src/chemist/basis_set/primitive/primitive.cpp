...
struct PrimitivePIMPL {
  ...
  bool operator==(const PrimitivePIMPL& rhs) const noexcept {
      ...
      auto rhs_state = std::tie(rhs.m_center, rhs.m_coefficient, rhs.m_exponent);
...
PRIM_TYPE::Primitive(const PRIM_TYPE& rhs) :
    Primitive(has_pimpl_() ? std::make_unique<pimpl_type>(*rhs.m_pimpl_) : nullptr) {}
...
template<typename T>
typename PRIM_TYPE::coefficient_reference 
PRIM_TYPE::coefficient() noexcept {
    if(!has_pimpl_()) m_pimpl_ = make_pimpl_();
    return m_pimpl_->m_coefficient;
}

template<typename T>
typename PRIM_TYPE::const_coefficient_reference 
PRIM_TYPE::coefficient() const noexcept {
    assert_pimpl_();
    return m_pimpl_->m_coefficient;
}
...
template<typename T>
typename PRIM_TYPE::const_exponent_reference 
PRIM_TYPE::exponent() const {
    assert_pimpl_();
    return m_pimpl_->exp();
}
...
template<typename T>
bool operator==(const PRIM_TYPE& rhs) const noexcept {
