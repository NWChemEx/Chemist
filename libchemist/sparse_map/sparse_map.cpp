...
#define PIMPL_COPY_CTOR(class_name, pimpl_type)                            \
    class_name::class_name(const class_name& rhs) :                        \
      m_pimpl_(std::make_unique<pimpl_type>(rhs.m_pimpl_ ? *rhs.m_pimpl_ : \
                                                           pimpl_type{})) {}
...
#define PIMPL_DTOR(class_name) class_name::~class_name() noexcept = default
...
size_type SparseMap::size() const noexcept { return m_pimpl_->m_sm.size(); }
...
index_set_map SparseMap::indices() const {
    index_set_map rv;
    for(const auto& [k, v] : *this) { rv[k] = indices(k); }
    return rv;
}
...
index_set_array SparseMap::indices(key_type ind) const {
    index_set_array rv;
    rv.reserve(dep_rank());
    for(size_type i = 0; i < dep_rank(); ++i) rv.push_back(indices(ind, i));
    return rv;
}

index_set SparseMap::indices(key_type ind, size_type mode) const {
    index_set rv;
    for(const auto& dep : at(ind)) rv.insert(dep[mode]);
    return rv;
}
...
SparseMap SparseMap::operator*(const SparseMap& rhs) const {
    ...
    for(const auto& [k, v] : lsm)
        if(rsm.count(k)) osm[k] = lsm.at(k) * rsm.at(k);
    return rv;
}
...
void SparseMap::check_rank_(size_type idx_rank, bool dep) const {
    ...
    throw std::runtime_error(iden + " indices have ranks of "s +
                             std::to_string(corr_rank) + " but idx with rank " +
                             std::to_string(idx_rank) + " was provided."s);
}

SparseMap SparseMap::inverse() const {
    SparseMap rv;
    for(const auto& x : *this) {
        for(const auto y : x.second) { rv.add_to_domain(y, x.first); }
    }
    return rv;
}
...
SparseMap SparseMap::chain(const SparseMap& sm) const {
    ...
    for(const auto& x : *this) {
        for(const auto y : x.second) {
            if(sm.count(y)) {
                for(const auto z : sm.at(y)) { rv.add_to_domain(x.first, z); }
            }
        }
    }
    return rv;
...
SparseMap SparseMap::map_union(const SparseMap& sm) const {
    if(!this->empty() && !sm.empty() &&
       (this->dep_rank() != sm.dep_rank() || this->ind_rank() != sm.ind_rank()))
        throw std::runtime_error(
          "Incompatible index ranks between maps for union");
    SparseMap rv = *this;
    for(const auto& x : sm) {
        for(const auto y : x.second) { rv.add_to_domain(x.first, y); }
    }
    return rv;
}
...
SparseMap SparseMap::intersection(const SparseMap& sm) const {
    if(!this->empty() && !sm.empty() &&
       (this->dep_rank() != sm.dep_rank() || this->ind_rank() != sm.ind_rank()))
        throw std::runtime_error(
          "Incompatible index ranks between maps for intersection");
    SparseMap rv;
    for(const auto& x : *this) {
        if(sm.count(x.first)) {
            for(const auto y : x.second) {
                if(sm.at(x.first).count(y)) { rv.add_to_domain(x.first, y); }
            }
        }
    }
    return rv;
