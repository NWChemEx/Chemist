...
inline auto condense_tensor_range(const SparseMap& sm,
                                  const TA::TiledRange& trange,
                                  const std::set<std::size_t>& ind_modes = {}) {
    ...
    std::map<key_type, std::map<key_type, TA::Range>> tile2old, tile2new;

    for(auto [ind, indices] : sm.indices()) {
        // Offsets only includes dep indices, bound has all
        auto [offsets, bound] =
          detail_::condense_tensor_range_(indices, trange, ind_modes);

        for(auto idx : sm.at(ind)) {
            range_vector old_range;
            for(size_type mode = 0, counter = 0; mode < rank; ++mode) {
                if(ind_modes.count(mode)) {
                    old_range.push_back(bounds[mode]);
                }
                else {
                    const auto dep_tile = idx[counter];
                    old_range.push_back(offsets[counter][dep_tile]);
                    ++counter;
                }
            }
            tile2old[ind][idx] = TA::Range(old_range);
            tile2new[ind][idx] = TA::Range(bound);
        }
    }
    return std::make_pair(tile2old, tile2new);
}
