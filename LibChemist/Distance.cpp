...
namespace LibChemist {
  ...
  inline std::vector<double> dist(const std::array<double,3> q1, const std::array<double,3> q2){
  return {mag(diff(q1,q2))};
}

inline std::vector<double> d1_dist(const std::array<double,3> q1,const std::array<double,3> q2)
{
...
inline std::vector<double> d2_dist(const std::array<double,3> q1,const std::array<double,3> q2)
{
  const double d=1/dist(q1,q2)[0];
  const auto dq=diff(q1,q2);
  const std::vector<double> dqdr=d1_dist(q1,q2);
  std::vector<double> deriv(36,0.0);
...
std::vector<double> Distance::deriv(size_t deriv_order,const Molecule& molecule,const std::vector<int>& coord_i)const{
...
std::vector<double> Pair13::deriv(size_t deriv_order,const Molecule& molecule,const std::vector<int>& coord_i)const{
  const std::vector<double> d=Distance::deriv(deriv_order,molecule,{coord_i[0],coord_i[2]});
