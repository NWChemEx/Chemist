...
//    center() = rhs.center();
//
//    //Only support assignment if we have no primitives or the same number as rhs
//    if(!nprims()) {
...
bool operator==(const AOShell& lhs, const AOShell& rhs) noexcept {
    if(lhs.nprims() != rhs.nprims() || lhs.pure() != rhs.pure() ||
       lhs.center() != rhs.center()) return false;

    for(size_type i = 0; i < lhs.nprims(); ++i)
        if(lhs.coef(i) != rhs.coef(i) || lhs.alpha(i) != rhs.alpha(i)) return
              false;
    return true;
}
...
std::ostream& operator<<(std::ostream& os, const LibChemist::AOShell& shell) {
    ...
    for(auto i = 0; i < shell.nprims(); ++i)
        os<<"c: " << shell.coef(i) << " alpha: " << shell.alpha(i) <<std::endl;
    return os;
}
